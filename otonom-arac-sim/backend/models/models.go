...
type Vehicle struct {
	...
	AssignedSpot  int           `json:"assignedSpot"`  // Rezerve edilmiş spot (-1 = rezervasyon yok)
}
...
// OptStats optimizasyon istatistiklerini tutar
type OptStats struct {
	TotalWaitTime    int `json:"totalWaitTime"`    // Toplam bekleme tick toplamı (tüm araçlar)
	CollisionAvoided int `json:"collisionAvoided"` // calculateOptimalRetreat ile kaçınılan çarpışma
	DeadlockResolved int `json:"deadlockResolved"` // resolveDeadlock ile çözülen deadlock sayısı
}
